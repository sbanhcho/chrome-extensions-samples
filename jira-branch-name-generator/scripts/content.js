var branchNameGenerator = function() {
    ...
    const ticketTitle = document.querySelector("h1#summary-val") ?? document.querySelector("h1");
    const ticketID = document.querySelector('[data-testid*="current-issue"] span') ?? document.querySelector('#key-val');
    const ticketIDText = ticketID.textContent;
    let ticketTypeAriaLabel = document.querySelector('[data-testid*=".change-issue-type"] button[aria-label]') ?? document.querySelector('#type-val img');
    let ticketTypeAriaLabelText = ticketTypeAriaLabel.getAttribute('aria-label') ?? ticketTypeAriaLabel.getAttribute('title');
    let ticketType;
    ...
    if (ticketTypeAriaLabelText.toLowerCase().includes("bug") || ticketTypeAriaLabelText.toLowerCase().includes("bogue")) {
        ticketType = "bugfix";
    }
    else if (ticketTypeAriaLabelText.toLowerCase().includes("tâche")) {
        ticketType = "task";
    }
    else {
        ticketType = "feature";
    ...
    if (ticketTitle) {
        ...
        let ticketDescName = ticketTitle.textContent.toLowerCase().trim().replace(/\s/g, '-');
        ticketDescName = replaceSpec(ticketDescName);
        ticketDescName = ticketDescName.replace(/[^a-z\-\d]/g, '');
        ...
        const heading2 = document.getElementById("collaboratorsmodule");
        (heading ?? heading2).insertAdjacentElement("afterend", generatorContainer);
        ...
        generatorContainer.innerHTML = `
            ...
            <div id="copy-to-clipboard-container">
                <fieldset>
                <input id="branch-name" value="${ticketType}/${ticketIDText}--${ticketDescName}"/>
